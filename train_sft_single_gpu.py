...
def list_local_checkpoints_recursive(root: Path) -> List[Path]:
    """Return finalized checkpoint directories found anywhere under root, sorted newest-first."""
    root = Path(root)
    if not root.exists() or not root.is_dir():
        return []
    candidates: List[Path] = []
    seen: set[str] = set()
    for entry in root.rglob("checkpoint-*"):
        if not entry.is_dir() or entry.name.endswith(".tmp"):
            continue
        if checkpoint_step_from_name(entry.name) < 0:
            continue
        if not (entry / "training_state.pt").exists():
            continue
...
def run_training(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        ...
        def fetch_micro_batch(global_sample_start: int) -> List[Dict[str, Any]]:
            ...
            for pos in range(global_sample_start, global_sample_start + local_batch_size):
        ...
        while step < total_steps:
            ...
            grad_norm_log = grad_norm

            if spike_monitor.update(step, mean_ce):
            ...
            if step % args.log_every == 0 or step == 1:
                global_tokens_since_log = tokens_since_log
            ...
            if step % args.val_every == 0 or step == total_steps:
                ...
                tqdm.write(f"  [val] step={step}  val_ce={last_val_ce:.4f}  val_acc={last_val_acc:.4f}")
