...
from ouroboros.coconut.dataset_runtime import (
    download_dataset_from_hub as _download_dataset_from_hub_module,
    get_max_stage as _get_max_stage_module,
    load_canonical_dataset as _load_canonical_dataset_module,
)
...
def _download_dataset_from_hub(
    ...
    _download_dataset_from_hub_module(
        data_dir,
...
def load_canonical_dataset(
    ...
    return _load_canonical_dataset_module(
        data_dir,
        max_samples,
        is_main_process=_is_main_process,
        download_from_hub=_download_dataset_from_hub,
    )


def get_max_stage(args: argparse.Namespace, stats: Dict[str, Any]) -> int:
    return _get_max_stage_module(args.max_stage, stats, is_main_process=_is_main_process)
...
def build_sample_at_stage(
    ...
    stage_sample = build_stage_sample(sample, int(stage_k))
    n_latent = len(stage_sample.latent_steps)
    remaining_steps = stage_sample.visible_steps
