...
| Kaggle launch matrix: per-mode command argv, output env key, notebook shell template, worker-id requirement, workflow label | `ouroboros.kaggle_launch_matrix` | `ouroboros.kaggle` compatibility builders, notebook literal `!torchrun` branches, GitHub Actions workflow YAML, dispatch metadata |
...
- Notebook launch branches still use literal IPython `!torchrun`; tests compare those literal commands against matrix shell templates instead of replacing them with dynamic Python execution.
...
| `tests/test_kaggle_launch_matrix.py` | matrix coverage for every launch mode, compatibility-builder parity, GPU/CPU intent, notebook shell-template drift, workflow exposure |
...
- For launch-mode policy questions, read `ouroboros.kaggle_contract` first. For launch behavior, read `ouroboros.kaggle_launch_matrix`; string literals in workflow/notebook code should mirror the matrix and stay covered by tests.
