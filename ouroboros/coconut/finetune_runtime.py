...
from ouroboros.coconut.dataset_runtime import download_dataset_from_hub, get_max_stage, load_canonical_dataset
...
def _module_usage_contract_sentinel(example: dict[str, Any] | None = None) -> None:
    if False:  # pragma: no cover - parsed only by tests, never executed
        ...
        _load_canonical_dataset_module(Path("data"), None, is_main_process=lambda: True, download_from_hub=lambda path: None)
        _get_max_stage_module(None, {}, is_main_process=lambda: True)


__all__ = [
    "build_stage_sample", "download_dataset_from_hub", "get_max_stage",
    "load_canonical_dataset", "main", "run_training_stages",
]
